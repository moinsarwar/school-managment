...
@section('content')
    <h4 class="fw-bold mb-4"><i class="bi bi-person-plus me-2"></i>New Admission</h4>
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('office.admissions.store') }}">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="section_id" class="form-select @error('section_id') is-invalid @enderror" required>
                            <option value="">Select</option>
                            @foreach($sections as $s)<option value="{{ $s->id }}" {{ old('section_id') == $s->id ? 'selected' : '' }}>{{ $s->name }} ({{ $s->schoolClass->name ?? '' }})</option>@endforeach
                        </select>
                ...
                <button type="submit" class="btn btn-info text-white"><i class="bi bi-check-lg me-1"></i> Admit
                    Student</button>
